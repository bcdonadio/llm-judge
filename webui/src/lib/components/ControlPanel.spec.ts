...
describe('ControlPanel', () => {
  ...
  it('hydrates fields from the defaults store once', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByLabelText('Models to evaluate')).toHaveValue('first\nsecond');
    });
  ...
  it('validates models before starting a run', async () => {
    ...
    expect(screen.getByText('Please provide at least one model slug.')).toBeInTheDocument();
  });
  ...
  it('submits parsed configuration to the backend', async () => {
    ...
    const limitInput = screen.getByLabelText('Prompt limit') as HTMLInputElement;
  ...
  it('handles backend errors for lifecycle actions', async () => {
    ...
    try {
      ...
      vi.spyOn(stores, 'pauseRun').mockResolvedValueOnce({ ok: false }).mockResolvedValueOnce({ ok: true });
      vi.spyOn(stores, 'resumeRun').mockResolvedValueOnce({ ok: false }).mockResolvedValueOnce({ ok: true });
      vi.spyOn(stores, 'cancelRun').mockResolvedValueOnce({ ok: false }).mockResolvedValueOnce({ ok: true });
  ...
  it('disables buttons according to the current run state', async () => {
    ...
    statusStore.set({ ...baseStatus, state: 'paused', config: { models: ['one'] } as RunConfig });
    await tick();
    expect(screen.getByRole('button', { name: 'Run' })).toBeDisabled();
