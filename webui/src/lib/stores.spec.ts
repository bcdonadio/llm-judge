...
import type { ArtifactsInfo, DefaultsResponse, ModelSummary, StatusPayload } from './types';
import {
  artifactsStore,
  cancelRun,
  connectEvents,
...
class MockEventSource {
  ...
  url: string;
  listeners: Record<string, ((event: MessageEvent<string> | Record<string, unknown>) => void)[]> = {};
  closed = false;
  ...
  addEventListener(type: string, listener: (event: MessageEvent<string> | Record<string, unknown>) => void) {
    this.listeners[type] ||= [];
...
describe('stores', () => {
  ...
  it('recovers from initialization errors', async () => {
    ...
    expect(errorSpy).toHaveBeenCalledWith('Failed to initialise web UI state', error);
    expect(get(scoreboardStore)).toEqual({});
    expect(get(artifactsStore)).toBeNull();
  ...
  it('routes server-sent events into the Svelte stores', () => {
    ...
    emit(instance, 'message', {
      type: 'message',
      payload: { role: 'assistant', content: 'Reply', model: 'm', prompt_index: 2, step: 'analysis' },
    });
    ...
    expect(get(messagesStore).some((msg) => msg.content === 'Judge decision recorded.')).toBe(true);

    emit(instance, 'summary', { type: 'summary', payload: { summary } });
    expect(get(scoreboardStore)).toEqual(summary);

    const completionArtifacts: ArtifactsInfo = { csv_path: 'complete.csv', runs_dir: 'dir' };
    emit(instance, 'run_completed', { type: 'run_completed', payload: { ...completionArtifacts, summary } });
    ...
    expect(get(messagesStore).some((msg) => msg.content === 'Run cancelled.')).toBe(true);

    emit(instance, 'artifacts', { type: 'artifacts', payload: { csv_path: 'direct.csv' } });
    ...
    expect(parseErrorSpy).toHaveBeenCalledWith('Failed to parse SSE payload', expect.anything());
  ...
  it('falls back when crypto.randomUUID is unavailable', () => {
    ...
    const disconnect = connectEvents();
    const instance = MockEventSource.instances.at(-1)!;
    emit(instance, 'message', { type: 'message', payload: { content: 'no crypto' } });
  ...
  it('posts JSON payloads for run lifecycle helpers', async () => {
    ...
    await expect(resumeRun()).resolves.toEqual({ ok: false, message: 'Request failed' });

    await expect(cancelRun()).resolves.toEqual({ ok: false, message: 'Network error' });
